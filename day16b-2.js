...
const main = () => {
	...
	let r = (time, pressure, opens, position1, route1, iteration1, dest1, position2, route2, iteration2, dest2, heuristic) => {
		heuristic = [...heuristic]
		opens = {...opens};
		let tick = () => {
			Object.entries(opens).forEach((e)=>e[1]?pressure+=pressures[e[0]]:0)
			time--;
			heuristic.push(pressure)
			if(time === 0)
			{
				if (pressure > maxp) {
					maxp = pressure;
					max_heuristic = heuristic
					console.log(maxp, max_heuristic)
				}
				return false;
			}
			for (let i = 0; i < Math.min(heuristic.length, max_heuristic.length); i++)
			{
				if(heuristic[i] < max_heuristic[i]) return false;
			}
			return true;
		}
		let tick2 = () => {
		...
		if (pressures[position1] === 0) opens[position1] = true;
		if (pressures[position2] === 0) opens[position2] = true;

		let p1_open = false
		let p2_open = false;

		if (opens[position1] === false){
			p1_open = true;
			// if (dest2 === position1) return;
		}

		if (opens[position2] === false){
			p2_open = true;
			// if (dest1 === position2) return;
		}

		if (p1_open && p2_open)
		{
			if (!tick()) return;
			opens[position1] = true
			opens[position2] = true
			if (!tick()) return;
			iteration1++;
			iteration2++;
		}
		...
		if (iteration1 < route1.length && iteration2 < route2.length)
		{
			r(time, pressure, opens, route1[iteration1], route1, iteration1, dest1, route2[iteration2], route2, iteration2, dest2, heuristic)
		}
		else if (iteration2 < route2.length)
		{
			let dests = Object.keys(paths[position1])
			let next_paths = Object.values(paths[position1])
			for(let i = 0; i < next_paths.length; i++)
			{
				if (opens[dests[i]] === false && dests[i] !== dest2)
					r(time, pressure, opens, next_paths[i][1], next_paths[i], 1, dests[i], route2[iteration2], route2, iteration2, dest2, heuristic)
			}
		}
		else if (iteration1 < route1.length)
		{
			let dests = Object.keys(paths[position2])
			let next_paths = Object.values(paths[position2])
			for(let i = 0; i < next_paths.length; i++)
			{
				if (opens[dests[i]] === false && dests[i] !== dest1)
					r(time, pressure, opens, route1[iteration1], route1, iteration1, dest1, next_paths[i][1], next_paths[i], 1, dests[i], heuristic)
			}
		}
		else
		{
			let dests1 = Object.keys(paths[position1])
			let next_paths1 = Object.values(paths[position1])
			let dests2 = Object.keys(paths[position2])
			let next_paths2 = Object.values(paths[position2])

			for(let i = 0; i < next_paths1.length; i++)
			{
				for(let j = 0; j < next_paths2.length; j++)
				{
					if (opens[dests1[i]] === false && opens[dests2[j]] === false && dests1[i] !== dests2[j])
						r(time, pressure, opens, next_paths1[i][1], next_paths1[i], 1, dests1[i], next_paths2[j][1], next_paths2[j], 1, dests2[j], heuristic)
				}
			}
		}
	}

	for(let i = 0; i < Object.keys(first).length-1; i++)
	{
		for(let j = i+1; j < Object.keys(first).length; j++)
			{
			let dests = Object.keys(first)
			let next_paths = Object.values(first)
			r(26, 0, opens, next_paths[i][0], next_paths[i], 0, dests[i], next_paths[j][0], next_paths[j], 0, dests[j], [])
			console.log(i, j, Object.keys(first).length)
		}
	}
