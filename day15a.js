...
const main = () => {
	...
	for (let i = 0; i < data.length; i++)
	{
		...
		distances.push(distance(sensorx, beaconx, sensory, beacony))
	}
	...
	for (let x = xmin-max_dist; x <= xmax+max_dist; x++)	
	{
		let posx = x;
		let posy = 2000000;
		let reached = false;
		...
		if (!beac)
		{
			for (let i = 0; i < data.length; i++)
			{
				let sensorx = data[i][0][0]
				let sensory = data[i][0][1]
				let dist = distance(sensorx, posx, sensory, posy)
				if (dist <= distances[i]) reached = true;
			}
		}
		if (reached && !beac) count++;
	}
